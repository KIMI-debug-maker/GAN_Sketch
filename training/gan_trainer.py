...
class GANTrainer():
    ...
    def train_one_step(self, data, iters):
        self.gan_model.set_requires_grad(False, True)#train discriminator
        self.run_discriminator_one_step(data)
        #d_reg
        if not self.opt.no_d_regularize:# and iters % self.opt.d_reg_every == 0:#angry, use d reg in every iter?
            self.run_discriminator_regularization_one_step(data)
        self.gan_model.set_requires_grad(True, False)#train generator
    ...
    def get_latest_losses(self):
        ...
        with jt.no_grad():
            res={k: v.mean() for k, v in self.reports.items()}
        return res
    ...
    def set_fixed_noise(self, device):
        ...
        if self.opt.reduce_visuals:
            sample_z_file = './cache_files/sample_z_reduced.pkl'
            interp_z_file = './cache_files/interp_z_reduced.pkl'
        else:
            sample_z_file = './cache_files/sample_z.pkl'
            interp_z_file = './cache_files/interp_z.pkl'
        ...
        if os.path.exists(interp_z_file):
            ...
            with jt.no_grad():
                ...
                z = jt.contrib.concat(z, 1).view(-1, 512)
