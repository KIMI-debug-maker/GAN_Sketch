...
class Upsample(nn.Module):
    ...
    def execute(self, input):
        out = upfirdn2d(self.kernel, up=self.factor, down=1, pad=self.pad)(input)
...
class Blur(nn.Module):
    ...
    def execute(self, input):
        out = upfirdn2d(self.kernel, pad=self.pad)(input)
...
class EqualLinear(nn.Module):
    def __init__(
        self, in_dim, out_dim, bias=True, bias_init=0, lr_mul=1, activation=False,norm=False
    ):
        super().__init__()
        #linear = 
...
class NoiseInjection(nn.Module):
    ...
    def execute(self, image, noise=None):
        if noise is None:
            ...
            #noise=dis.sample((batch, 1, height, width))
...
class StyledConv(nn.Module):
    def __init__(
        ...
        # self.bias = nn.Parameter(jt.zeros(1, out_channel, 1, 1))
...
class ToRGB(nn.Module):
    def __init__(self, in_channel, style_dim, upsample=True,blur_kernel=[1,3,3,1]):
        ...
        self.bias = jt.zeros((1, 3, 1, 1))#use a bias to learn color!!!
...
class Generator(nn.Module):
    def __init__(
        ...
        ### not used in common training, therefore it trigger a warning
        self.channels = {
            4: 512,
            8: 512,
            16: 512,
    ...
    def execute(
        ...
        noise=None,#different from styles????
        ...
        if len(styles) < 2:
            ...
            if styles[0].ndim < 3:
                latent = jt.float32(styles[0]).unsqueeze(1).repeat(1, inject_index, 1)
...
class Discriminator(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1]):
        ...
        self.final_linear=nn.Sequential(
            EqualLinear(channels[4] * 4 * 4, channels[4], activation=True,norm=True),
    ...
    def execute(self, input):
        ...
        out = jt.contrib.concat([out, stddev], 1)#concatenate
...
def conv_transpose2d(input, weight, padding=0, stride=2,groups=1):
    #N,C,H,W = (1, batch * in_channel, height, width)##input shape
